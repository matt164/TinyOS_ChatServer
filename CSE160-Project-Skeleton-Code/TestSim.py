...
class TestSim:
    ...
    def testClient(self, addr, srcPort, dest, dstPort, transfer):
        self.sendCMD(self.CMD_TEST_CLIENT, addr, "{0}{1}{2}{3}".format(chr(srcPort),chr(dest),chr(dstPort),chr(transfer)));

    def testServer(self, addr, port):
        self.sendCMD(self.CMD_TEST_SERVER, addr, "{0}".format(chr(port)));

    def appServer(self, addr, port):
        self.sendCMD(self.CMD_APP_SERVER, addr, "{0}".format(chr(port)));

    def appClient(self, addr, port):
        self.sendCMD(self.CMD_APP_CLIENT, addr, "{0}".format(chr(port)));

    def sendChatCommand(self, addr, cmd):
        self.sendCMD(self.CMD_SEND_CHAT_COMMAND, addr, "{0}".format(cmd));

    #def hello(self, addr, username):
        #self.sendCMD(self.CMD_HELLO, addr, "{0}".format(username));

    #def tellAll(self, addr, msg):
        #self.sendCMD(self.CMD_TELL_ALL, addr, "{0}".format(msg));

    #def tell(self, addr, recipient, msg):
        #self.sendCMD(self.CMD_TELL, addr, "{0}{1}".format(recipient, msg));

    #def listUsers(self, addr):
        #self.sendCMD(self.CMD_LIST_USERS, addr, "connected users");
