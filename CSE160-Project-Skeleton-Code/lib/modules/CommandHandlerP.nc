...
implementation{
    task void processCommand(){
        if(! call Queue.empty()){
            ...
            case CMD_APP_CLIENT:
                dbg(COMMAND_CHANNEL, "Command Type: Chat Client\n");
                signal CommandHandler.setAppClient(buff[0]);
                break;

            case CMD_SEND_CHAT_COMMAND:
                dbg(COMMAND_CHANNEL, "Command Type: Chat Client Command\n");
                signal CommandHandler.sendChatCmd(&buff[0]);
                break;

            /*case CMD_HELLO:
                dbg(COMMAND_CHANNEL, "Command Type: Set Username\n");
                signal CommandHandler.hello(&buff[0]);
                break;

            case CMD_TELL_ALL:
                dbg(COMMAND_CHANNEL, "Command Type: Msg All\n");
                signal CommandHandler.tellAll(&buff[0]);
                break;

            case CMD_TELL:
                dbg(COMMAND_CHANNEL, "Command Type: Whisper\n");
                signal CommandHandler.tell(&buff[0],&buff[1]);
                break;
