...
interface Transport{
    ...
    */
   command socket_t accept(socket_t fd, socket_port_t port);

   /**
    * Send a TCP packet struct encapsulated in a packet to the destination 
    * specified in the header. Needs to be assigned a valid seqNum for the
    * node before being sent over network.
    * @param
    *    pack package: pack object containing the TCP struct
    * @side Client/Server
    */

    command void initTransport();

    command enum socket_state checkState(socket_t fd);

    event void addClient(socket_t serverFd);

    event void send(pack* package);
